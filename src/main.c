  ...
  json_object_object_foreach(jobj, key, val) { /*Passing through every array element*/
    ...
    switch (type) {
      ...
      case json_type_string: 
			    ...
			    print_json_value(val);
			    strcpy(location, json_object_get_string(val));
...
int checkString(char *str)
{
    ...
    while (*str) {
		...
		printf("\n\nNot a valid city name (letters only)!\n\n");
		exit(1);	
		}
        str++;
...
if ( argc != 2 ) 
	...
	struct json_object *jobj = json_tokener_parse(chunk.memory);
   	...
   	json_parse(jobj);
    	...
    	if (strcmp(toLower(city),toLower(location)) !=0){
		printf("\n\nEntered city name does not match with server response! Check city name!\n\n");
	}
