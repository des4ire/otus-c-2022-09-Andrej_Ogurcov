...
static const off_t DATA_SIZE = 1073741000;
...
int main(int argc, char *argv[]) {
    ...
    if (fstat(file_descriptor, &status_buffer) < 0) {
        perror("File error");
        exit(EXIT_FAILURE);
    }
	
    uint32_t CRC32 = 0xFFFFFFFFu;
	off_t len = DATA_SIZE & ~(sysconf(_SC_PAGE_SIZE) - 1);
   
    size_t counter = status_buffer.st_size / len;
    ...
    for (size_t i = 0; i < counter; i++) {
        file_pointer = mmap(NULL, len, PROT_READ, MAP_SHARED, file_descriptor, offset);
        if (file_pointer == MAP_FAILED) {
        	...
        	for (int64_t j = 0; j < len; j++) {
            	const uint32_t Index = (CRC32 ^ file_pointer[j]) & 0xff;
            	CRC32 = (CRC32 >> 8) ^ CRC_list[Index];
        	}

       	offset += len;
	munmap(file_pointer, len);
    }

    
    size_t rem = status_buffer.st_size - offset;
    
    file_pointer = mmap(NULL, rem,PROT_READ, MAP_SHARED, file_descriptor, offset);
    if (file_pointer == MAP_FAILED) {
